...
CRITICAL FIXES:
1. Enabled WAL (Write-Ahead Logging) mode for concurrent access
2. Reduced connection timeout to avoid pile-up
3. Changed isolation_level to DEFERRED for better locking
4. Added connection reuse to reduce lock contention
"""
...
def init_db(db_path: str = DB_PATH):
    ...
    # ✅ CRITICAL FIX: Enable WAL mode for concurrent writes
    cur.execute("PRAGMA journal_mode=WAL")
    cur.execute("PRAGMA synchronous=NORMAL")  # Faster writes
    cur.execute("PRAGMA busy_timeout=5000")   # 5 second timeout on lock
    ...
    conn.commit()
    conn.close()
    logger.info(f"Database initialised at {db_path} with WAL mode enabled")
...
class FeedSimulator:
    ...
    - DEFERRED isolation level for better lock management
    - Shorter timeout (5s) to fail fast instead of hanging
    - Connection reuse to avoid repeated lock attempts
    """
    ...
    def _ensure_wal_mode(self):
        ...
        try:
            ...
            conn.close()
            logger.info(f"WAL mode enabled: {wal_result}")
    ...
    def insert_batch(self, batch_size: int = BATCH_SIZE) -> int:
        ...
        KEY CHANGES:
        - Uses DEFERRED isolation level (readers don't block writers)
        - Timeout reduced to 5 seconds (fail fast)
        - Single connection for entire batch (not per-row)
        ...
        for attempt in range(max_retries + 1):
            try:
                # ✅ FIX: Use 5-second timeout (not 10) to fail fast
                # ✅ FIX: Use DEFERRED isolation for better concurrency
                conn = sqlite3.connect(self.db_path, timeout=5.0)
                conn.isolation_level = "DEFERRED"  # Changed from None (autocommit)
                cur = conn.cursor()
                ...
                for _, row in batch.iterrows():
                    try:
                        ...
                        logger.debug(f"Skipped {row['txn_id']}: {e}")

                # ✅ FIX: Single commit after all rows (not per-row)
                conn.commit()
                conn.close()
    ...
    def get_unprocessed(self, limit: int = 50) -> list[dict]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch unprocessed transactions: {e}")
            return []
    ...
    def enqueue_suspect(self, conn, txn: dict, result):
        ...
        try:
            ...
            logger.error(f"Failed to enqueue suspect {txn['txn_id']}: {e}")
    ...
    def run_loop(self, batch_size: int = BATCH_SIZE, interval: int = POLL_INTERVAL):
        ...
        print(f" Database: {self.db_path} (WAL mode enabled)")
